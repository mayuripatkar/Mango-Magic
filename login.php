...
if(isset($_POST['email']) && isset($_POST['password'])){
    ...
    $row = mysqli_fetch_array($run_query);
		...
		#Cookie is created in login_form.php so if that cookie is available means user is not login
	
	if($count == 1)#user record is available
	{
		if (isset($_COOKIE["product_list"])) 
		{
			$p_list = stripcslashes($_COOKIE["product_list"]);
				#removes backslashes() added by the addclashes()
				$product_list = json_decode($p_list,true);
				for ($i=0; $i < count($product_list); $i++) 
				{ 	
				#to check if user cart item if there is any product listed
					$verify_cart = "SELECT id FROM cart WHERE user_id = $_SESSION[uid] AND p_id = ".$product_list[$i];
					$result  = mysqli_query($con,$verify_cart);
					if(mysqli_num_rows($result) < 1)
					{
						#to update userid with valid id in db if user is adding product first time in cart
						$update_cart = "UPDATE cart SET user_id = '$_SESSION[uid]' WHERE ip_add = '$ip_add' AND user_id = -1";
						mysqli_query($con,$update_cart);
					}
					else
					{
						#if already that product is available into db table we will delete that record
		...
		else
		{
            ...
            $run_query = mysqli_query($con,$sql);
            $count = mysqli_num_rows($run_query);
            #if user record is available in db then $count will be equal to 1
            if($count == 1){
                $row = mysqli_fetch_array($run_query);
                $_SESSION["uid"] = $row["admin_id"];
                $_SESSION["name"] = $row["admin_name"];
                $ip_add = getenv("REMOTE_ADDR");
                #As earlier to check user record
